    ...
    for (let i = obstacles.length - 1; i >= 0; i--) {
        ...
        for (let j = playerProjectiles.length - 1; j >= 0; j--) {
            ...
            if (checkCollision(b, o)) {
                playerProjectiles.splice(j, 1); o.hp -= (weaponLevel === 1 ? 1 : 3);
...
function draw() {
    ...
    playerProjectiles.forEach(p => { ctx.fillStyle = p.color; ctx.fillRect(p.x, p.y, p.w, p.h); });
    if (bossActive) {
    ...
    bossProjectiles.forEach(b => { ctx.fillStyle = "#32CD32"; ctx.beginPath(); ctx.arc(b.x, b.y, b.r, 0, 7); ctx.fill(); });
    powerUps.forEach(p => { ctx.fillStyle = "#ff00ff"; ctx.fillRect(p.x, p.y, p.w, p.h); });
    ...
    if (isPaused) {
        ...
        ctx.fillText("PAUSA", 400, 200); ctx.textAlign = "start";
    }
}
...
window.addEventListener('keydown', e => {
    ...
    keys[e.code] = true;
    if ((e.code === 'Space' || e.code === 'ArrowUp') && gameActive && (player.grounded || player.jumpCount < 2)) { 
...
async function finalizarJuego() {
    ...
    let nombreUsuario = prompt("FIN DE MISIÓN. PTS: " + score + "\nNOMBRE:");
    if (nombreUsuario) {
        try {
